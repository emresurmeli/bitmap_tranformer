...
fs.readFile('./img/bitmap1.bmp', function (err, buff) {
  if (err) throw err;
  console.log(buff);
  // Convert buffer into a Javascript Object and log it into the console
  var buffObj = new bitmap.bitMapData(buff);
  // Create empty array to represent the color pallete
  var pallete = [];
  // Use a loop to extract the color pallete data into an array of arrays as such: [b, g, r, a]
  var readColorPallete = function (buff) {
    var counter = 0;
    // Between 54 - 182 bits in the pallete is where all the non black/white colors live
  ...
  // Now lets use the loop to read the color pallete and copy it into the pallete array 
  ...
  buff.write(transformedPallete, 54, 182);
  // Not write the new buffer data into a new .bmp file and you have a randomized .bmp file
